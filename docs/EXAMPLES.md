...
def run_pendulum_comparison():
    ...
    methods = {
        ...
        "MPPI + Adam": {"acceleration": "adam", "lr": 0.1},
        "MPPI + NAG": {"acceleration": "nag", "lr": 0.1, "momentum": 0.9},
        "MPPI + RMSprop": {"acceleration": "rmsprop", "lr": 0.1}
    }
    ...
    for name, params in methods.items():
        ...
        final_angle = torch.atan2(trajectory[-1, 1], trajectory[-1, 0])
        
        results[name] = {
            "trajectory": trajectory.cpu(),
            "control": optimal_control.cpu(),
            "final_angle": final_angle.cpu() * 180 / 3.14159,
            "solve_time": solve_time
        }
        
        print(f"  Final angle: {results[name]['final_angle']:.1f}°")
