...
class DiffMPPI:
    ...
    def solve(
        ...
        for iteration in range(num_iterations):
            ...
            candidate_controls = self.control_sequence.unsqueeze(0) + noise
            ...
            costs = self._evaluate_trajectories(initial_state, candidate_controls)
            
            # Compute weights using softmax
            weights = F.softmax(-costs / self.temperature, dim=0)
            
            # Update control sequence
            if self.acceleration is None:
                # Standard MPPI update
                self.control_sequence = torch.sum(
                    weights.unsqueeze(-1).unsqueeze(-1) * candidate_controls, dim=0
                )
            else:
                # Gradient-based update
                gradient = torch.sum(
                    weights.unsqueeze(-1).unsqueeze(-1) * noise, dim=0
                )
                self._apply_acceleration(gradient)
            
            # Track best cost
            current_cost = torch.min(costs).item()
        ...
        return self.control_sequence.detach()
